...
assert_contains "$test_workflow" 'types: [published]' 'handmatig gepubliceerde releases starten de notes-controle'
assert_contains "$test_workflow" '--generate-notes' 'automatische release notes worden gebruikt bij nieuwe releases'
assert_not_contains "$test_workflow" 'Automatisch genummerde versie van main op commit' 'de vaste nietszeggende releasebody is verwijderd'
assert_contains "$test_workflow" 'releases/generate-notes' 'handmatige releases halen GitHub release notes op'
assert_contains "$test_workflow" 'lxdg-generated-release-notes' 'handmatige release notes krijgen een herkenbare toevoegmarkering'
assert_contains "$test_workflow" '--method PATCH' 'handmatige tekst wordt behouden en aangevuld via een update'
